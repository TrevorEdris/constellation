...
Use this template when dispatching an implementer subagent. Fill every bracket
before dispatching. Paste the full task text — never tell the subagent to read a
plan file.
...
`model: opus`. On Codex, this maps to `spawn_agent` (see
`_shared/platform/codex-tools.md`).
...
Task:
  ...
  prompt: |
    ...
    [FULL TEXT of task from plan - paste it here, don't make the subagent read a file]
    ...
    **Ask them now.** Raise any concerns before starting work.
    ...
    2. Write tests first (RED-GREEN-REFACTOR) — tests must drive the REAL code
       path, not fakes or injected state
    3. Verify implementation works by running it, not by reasoning about it
    4. Commit your work
    ...
    **While you work:** If you encounter something unexpected or unclear, **ask
    questions**. It's always OK to pause and clarify. Don't guess or assume.
    ...
    **Completeness:** Did I fully implement the spec? Miss any requirements? Handle edge cases?
    **Quality:** Is this my best work? Are names clear? Is the code maintainable?
    **Discipline:** Did I avoid overbuilding (YAGNI)? Build only what was requested? Follow existing patterns?
    **Testing:** Do tests verify real behavior (not just mocks)? Did I follow TDD? Are tests comprehensive?
    ...
    - What you tested and the test results (paste real output)
    - Files changed
